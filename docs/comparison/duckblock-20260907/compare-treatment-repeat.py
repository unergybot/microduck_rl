...
old={(c['scenario'],c['seed']):c for c in a['cases'] if c['candidateId']=='browser-handoff'}
new={(c['scenario'],c['seed']):c for c in b['cases'] if c['candidateId']=='walking-handoff'}
result={'oldCandidate':'browser-handoff','repeatCandidate':'walking-handoff','caseCount':len(old),'sameCaseKeys':old.keys()==new.keys(),'statusReasonMatches':0,'exactMetricMatches':0,'exactCommonCsvMatches':0,'comparedCsvRows':0,'comparedCsvCells':0,'differences':[]}
