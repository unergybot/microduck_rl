...
EVIDENCE = ROOT / 'run-evidence' / ('eval-d959089-' + STAMP)
EVIDENCE.mkdir()
...
config = json.loads((ROOT/'input/comparison.json').read_text())
save('inputs.json',dict(experimentId=config['experimentId'],configSha256='sha256:'+hashlib.sha256((ROOT/'input/comparison.json').read_bytes()).hexdigest(),evaluatorSha256='sha256:'+hashlib.sha256((ROOT/'source-d959089/scripts/evaluate_sitstand_comparison.py').read_bytes()).hexdigest(),codeRevisions=config['codeRevisions']))
common = ['docker','run','-d','--user','10001:10001','--cpus','4','--memory','8g','--memory-swap','8g','--gpus','all','--network','none','--read-only','--tmpfs','/tmp:rw,size=536870912','--workdir','/tmp','--env','MUJOCO_GL=egl','--env','PYTHONPATH=/experiment/src','--env','MICRODUCK_ROM_BEARER_TOKEN_FILE=','--mount',f'type=bind,src={ROOT}/source-d959089,dst=/experiment,readonly','--mount',f'type=bind,src={ROOT}/input,dst=/input,readonly','--mount',f'type=bind,src={ROOT}/output,dst=/output']
...
try:
    ...
    execute('microduck-comparison-eval-'+STAMP.lower(),['/experiment/scripts/evaluate_sitstand_comparison.py','--config','/input/comparison.json','--output-root','/output'])
finally:
    after = snapshot()
