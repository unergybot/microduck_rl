...
config = json.loads((ROOT/'input/handoff.json').read_text())
save('inputs.json',dict(experimentId=config['experimentId'],configSha256='sha256:'+hashlib.sha256((ROOT/'input/handoff.json').read_bytes()).hexdigest(),evaluatorSha256='sha256:'+hashlib.sha256((ROOT/'source-6fc54a0/scripts/evaluate_sitstand_comparison.py').read_bytes()).hexdigest(),codeRevisions=config['codeRevisions']))
...
try:
    ...
    execute('microduck-comparison-eval-'+STAMP.lower(),['/experiment/scripts/evaluate_sitstand_comparison.py','--config','/input/handoff.json','--output-root','/output'])
finally:
    after = snapshot()
